...
bg = 'light'
imdb = get_imdb("voc_2007_"+bg+"testval")
roidb = imdb.roidb
...
if __name__ == '__main__':
    ...
    while count < DATASET_SIZE:
        ...
        seg_images_path = os.sep.join([images_path, seg_index[seg_idx]+'.jpg'])
        seg_img = Image.open(seg_images_path)

        seg_np = np.asarray(seg)
        obj_idx = random_obj_idx(set(seg_np.flatten()))
        obj_list = list(set(seg_np.flatten()))  # randomly pick an obj from seg img
        mask2 = (seg_np == obj_list[obj_idx])
        ...
        while(max_x - min_x) * (max_y - min_y) < img.size[0] * img.size[1] * 0.005 or \
            ...
            seg_np = np.asarray(seg)
            obj_idx = random_obj_idx(set(seg_np.flatten()))
            obj_list = list(set(seg_np.flatten()))
            mask2 = (seg_np == obj_list[obj_idx])
            min_x, max_x, min_y, max_y = find_obj_vertex(mask2)
        ...
        # img.show()
        # new_img.show()
        new_img.save(os.sep.join([save_imgage_path,'Test','test'+bg+str(count)+'.jpg']))  # save
        modify_xml(os.sep.join([image_annotation_path, image_index[img_idx] + '.xml']),
                   os.sep.join([save_annotation_path, 'Test','test'+bg+str(count)+ '.xml']),
